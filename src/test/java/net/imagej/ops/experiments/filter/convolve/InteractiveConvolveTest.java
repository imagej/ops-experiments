...
public class InteractiveConvolveTest {
	...
	public static <T extends RealType<T> & NativeType<T>> void main(final String[] args) throws IOException {
		...
		final FloatPointer x = ConvertersUtility.ii2DToFloatPointer(Views.zeroMin(extendedImg));
		
		// convert to FloatPointer
		final FloatPointer h = ConvertersUtility.ii2DToFloatPointer(Views.zeroMin(extendedKernel));
		...
		final FloatPointer X_ = new FloatPointer(2 * (extendedImg.dimension(0) / 2 + 1) * extendedImg.dimension(1));

		final FloatPointer H_ = new FloatPointer(2 * (extendedImg.dimension(0) / 2 + 1) * extendedImg.dimension(1));

		
		MKLConvolveWrapper.mklConvolve(x, h, X_, H_, (int)extendedImg.dimension(0), (int)extendedImg.dimension(1));
		
		final float[] convolved = new float[(int)(extendedImg.dimension(0)*extendedImg.dimension(1)) ];
		...
		long[] test=new long[]{extendedImg.dimension(0), extendedImg.dimension(1)};
